...
const lewy = document.getElementById('left')
const obrot = document.getElementById('rotate')
const wdol = document.getElementById('down')
const prawy = document.getElementById('right')
const startBtn = document.querySelector('.button')
const koloryKlockow = ['red', '#00BFA5', '#2979FF', 'yellow', ' darkviolet', 'lime', 'orange']
const wysokosc = 15
const odstep = 10
let Rotation = 0
let nextRandom = 0
...
//utworz siatke z oczkami
function creatDiv() {
  for (let i = 0; i < (wysokosc * odstep); i++) {
    let gridElement = document.createElement("div")
    grid.appendChild(gridElement)
  }
  for (let i = 0; i < odstep; i++) {
    let gridElement = document.createElement("div")
    gridElement.setAttribute("class", "oko3")
    grid.appendChild(gridElement)
  }
}

//tworzymy tablice z oczek siatki
creatDiv()
...
startBtn.addEventListener('click', () => {
  if (timerId) {
    ...
    lewy.addEventListener('click', moveleft)
    obrot.addEventListener('click', rotate)
    wdol.addEventListener('click', moveDown)
    prawy.addEventListener('click', moveright)
...
function dodajPunkty() {
  for (i = 0; i < odstep * wysokosc; i += odstep) {
    ...
    if (linia.every(index => oko[index].classList.contains('oko2'))) {
      punkty += 10
      linijka += 1
      punktacja.innerHTML = 'Punkty: ' + punkty
      linie.innerHTML = 'Linie:' + linijka
      linia.forEach(index => {
